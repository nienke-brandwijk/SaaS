describe.only('Calculator page - basic calculator correctness', () => {
  ...
  it.only('calculates yarn amount correctly and allows save', () => {
    ...
    cy.contains('h1', 'Yarn Amount Calculator').closest('.card').within(() => {
      ...
      cy.contains('button', 'Calculate').should('not.be.disabled').click();
    });

    cy.contains('h2', 'Yarn Amount Result').should('be.visible');
    cy.contains('You need 7 ball(s) of your yarn').should('exist');
    cy.contains('333 grams').should('exist');

    // Save button should always be enabled 
  ...
  it.only('calculates gauge swatch correctly', () => {
    ...
    cy.contains('h2', 'Gauge Swatch Result').should('be.visible');
  ...
  it.only('calculates picked stitches correctly', () => {
    ...
    cy.contains('h2', 'Picked Stitches Result').should('be.visible');
    cy.contains('72 stitches').should('exist');
    cy.contains('0.82').should('exist');

    cy.contains('button', 'Close').click();
    cy.contains('h2', 'Picked Stitches Result').should('not.exist');
  });
  ...
  it.only('gauge swatch allows saving when name provided', () => {
    ...
    // Save button should always be enabled now
    cy.contains('button', 'Save').should('not.be.disabled');
